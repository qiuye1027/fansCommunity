...
def countWords(lineStr):
    # article = open(filename, "r").read()
    words = jieba.cut(lineStr, cut_all=False)
    word_freq = {}
    for word in words:
...
class RNN:
    ...
    def __init__(self):
        ...
        # 输入层到隐含层的权重矩阵(100, 6000)
        self.Wih = np.random.uniform(-np.sqrt(1. / wordDim), np.sqrt(1. / wordDim), (hiddenDim, wordDim))

        # 隐含层到隐含层的权重矩阵(100, 100)
        self.Whh = np.random.uniform(-np.sqrt(1. / self.hiddenDim), np.sqrt(1. / self.hiddenDim), (hiddenDim, hiddenDim))

        # 隐含层到输出层的权重矩阵(10, 1)
        self.Why = np.random.uniform(-np.sqrt(1. / self.hiddenDim), np.sqrt(1. / self.hiddenDim), (wordDim, hiddenDim))
